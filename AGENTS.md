...
| **SideQuest Agent** | `SideQuest.md` | Execute one-off tasks with minimal overhead; bypass standard pipeline. | `agents/SideQuest.md` |
| **Implementer Agent** | `Implement.md` | Implement code from plan; document, reflect, and maintain implementation log. | `agents/Implement.md` |
...
### 2. Standard Handoffs
Agents are workflow-aware. Upon successful completion of a task, an agent SHALL indicate the next logical step and suggest the appropriate agent to the user (e.g., "**Work Item** complete. You may now invoke the **Planner Agent**"). Handoffs typically end with the code review step executing the **Complete** action.

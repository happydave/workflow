...
- **Work Items vs. Tickets**: We use the term **Work Item** to describe the conceptual unit of work (feature, bug, task). The term **Ticket** is reserved for the physical reference identifier. Files created within specific work item folders must use **lowercase-kebab-case** and the work item's root document is always named `workitem.md` (e.g., `workitem.md`, `plan.md`, `code.md`, `debug.md`). This makes them visually distinct from the framework files.
...
- **Artifact-Borne Authorization**: In multi-session work, authorization derives from what a session can verify for itself — committed artifacts (a project's standing authorization, a dispatch brief's decision-authority section) and the owner's own messages — not from peer attestation. A relayed approval degrades into hearsay with each hop, and a fleet's natural failure mode is well-meaning authorization proxying: each session honestly reports what it was told, yet the chain as a whole manufactures authority no one granted. Peers therefore carry *pointers* to authorization (cite the committed brief), decide only that work continues on its merits, and leave owner questions open rather than closing them by proxy. (Observed 2026-09-05: a session receiving an owner's "sounds good" through a two-hop relay correctly treated it as unverifiable and proceeded on merits while hedging — the fix is committed artifacts, not better relay wording.)
- **Publication Boundary**: The repository holds only what could be pushed to a stranger. Facts that are true at one site — hosts, addresses, accounts, paths, a private forge, personas that exist only there — live in a private site overlay (`tickets/site/`), and the framework refers to them by role (the private forge, the realm hosts, the ops persona). The overlay's term list is what the `WorkflowChange.md` leak gate reads, so the boundary is checked on every change rather than remembered. (Adopted 2026-09-12 after a private host address reached the public remote inside a knowledge document.)
- **Agent Task Suitability**: Subagents are appropriate for tasks where the output is grounded and verifiable by inspection — file moves, code changes that will be diff'd, search results. They are poorly suited to analytical tasks (plan review, code review) where prose output can be fabricated without reading the actual artifacts: a cold-start agent has no conversation context, receives only a folder path, and can pattern-match from its role definition to produce structurally correct but factually invented output. For analytical review steps, inline execution by the orchestrating session is preferred — it already has context, has already read relevant files, and does not have this trust gap.
